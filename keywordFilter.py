...
from keywordLoader import is_match
...
def filter_helper_v2(msg_id, include_all_compiled, exclude_any_compiled, matching_msg_id):
    try:
        service = get_gmail_service()
        full_msg = get_message(service, msg_id)
        payload = full_msg.get('payload', {})
        ...
        print(f"⚠️ Error in filter_helper_v2 with msg_id {msg_id}: {e}")
...
def filter_messages_by_keywords_v2(service, include_all_compiled, exclude_any_compiled, max_total=NUM_MESSAGES, max_threads=8):
    """
    Multi-threaded Gmail keyword filter using ThreadPoolExecutor.
    Fetches all messages via pagination (list_messages) and scans them in parallel.
    """
    ...
    try:
        ...
        # Use ThreadPoolExecutor to parallelize
        with ThreadPoolExecutor(max_workers=max_threads) as executor:
            futures = [executor.submit(filter_helper_v2, msg['id'], include_all_compiled, exclude_any_compiled, matching_msg_id) for msg in messages]

        return matching_msg_id

    except Exception as e:
        print(f"⚠️ Error in filter_messages_by_keywords_v2: {e}")
        return []